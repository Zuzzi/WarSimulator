...
public class AI {
	...
	public static  String decidi (String truppeA, String truppeB) {
		...
		for (int i = 0; i<5 && !trovato; i++) {
		    ...
		    if (neurone1.attivato && neurone2.attivato && neurone3.attivato && neurone4.attivato && neurone5.attivato) trovato = true;

		    
		}
		
		if (trovato) strategiaAI = strategie.get(k);
		else if (!trovato && tipoTruppeB.equals("tiro")) strategiaAI = "tenere la posizione";
		else if (!trovato && tipoTruppeB.equals("fanteria")) strategiaAI = "attacco frontale";
		else if (!trovato && tipoTruppeB.equals("cavalleria")) strategiaAI = "attacco a cuneo";
