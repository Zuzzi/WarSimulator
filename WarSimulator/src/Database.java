...
public class Database {
	...
	static boolean esistenzaTabella() throws SQLException{
				...
				int count=0;
				 ...
				 String query =
					        "select count(*)as count from information_schema.tables  where table_name ='SALVATAGGIO'";
				...
				try{
					...
					infoBoxErrore("Si è verificato un errore durante il controllo sull'esistenza della tabella, il programma verrà chiuso.","Errore");
					System.exit(0);
	...
	static  void stampaTabella(JTable tabellone)throws SQLException{
		...
		int grandezza = 0;
		 ...
		 String header[] = new String[] { "Salvataggio", "Terreno", "Ora",
		            "Vincitore", "Popolo1", "Generale1", "Truppe1", "Strategia1", "Popolo2", "Generale2", "Truppe2", "Strategia2" };
		 ...
		 tabellone.setModel(dtm);
					...
					try{				
						...
						while (rs.next()) {
							...
							dtm.addRow(new Object [] {rs.getString("partita"),rs.getString("terreno"),rs.getString("ora"),rs.getInt("vincitore"),rs.getString("popolo1"),
									rs.getString("generale1"),rs.getString("truppe1"),rs.getString("strategia1"),rs.getString("popolo2"),rs.getString("generale2"),rs.getString("truppe2"),rs.getString("strategia2")});
		...
		infoBoxErrore("Si è verificato un errore durantela stampa della tabella, il programma verrà chiuso.","Errore");
		System.exit(0);
	 ...
	 static  void esegui_query(String query) throws SQLException {
				...
				try{
					...
					infoBoxErrore("Si è verificato un errore durante l'esecuzione di una query, il programma verrà chiuso.","Errore");
	 ...
	 public static  boolean salvataggio_esistente(String nomePartita) throws SQLException{
		 		...
		 		int count=0;
		 		String salvataggio=
		 				"select count(*)as count FROM SALVATAGGIO WHERE partita='"+nomePartita+"';";
		 		...
		 		try{
		 			...
		 			infoBoxErrore("Si è verificato un errore durante il controllo di un salvataggio già esistente, il programma verrà chiuso.","Errore");
		 			System.exit(0);
